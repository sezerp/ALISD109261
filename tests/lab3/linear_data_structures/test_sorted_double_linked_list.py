...
from lab3.linear_data_structures.sorted_double_linked_list import SortedDoubleLinkedList


class TestSortedDoubleLinkedList(unittest.TestCase):
    def test_insert_single_element(self):
        # given
        xs = SortedDoubleLinkedList[int]()
        # when
        xs.insert(1)
        # then
    ...
    def test_insert_multiple_elements(self):
        ...
        self.assertEqual(1, xs.head.next.previous.value)

        self.assertEqual(3, xs.head.next.next.value)
        self.assertEqual(2, xs.head.next.next.previous.value)

        self.assertEqual(4, xs.head.next.next.next.value)
        self.assertEqual(3, xs.head.next.next.next.previous.value)

        self.assertEqual(5, xs.head.next.next.next.next.value)
        self.assertEqual(4, xs.head.next.next.next.next.previous.value)
    ...
    def test_delete_multiple_elements(self):
        ...
        self.assertEqual(None, xs.tail.next)
        self.assertEqual(3, xs.tail.previous.value)
